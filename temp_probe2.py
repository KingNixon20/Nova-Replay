...
candidates = []
# Try (a{sv}) variants with different token keys and values
def make_tuple_with_map(d):
    return GLib.Variant('(a{sv})', (d,))

candidates.append(("(a{sv}) empty dict", lambda: make_tuple_with_map({})))
candidates.append(("(a{sv}) with session_handle_token empty string", lambda: make_tuple_with_map({'session_handle_token': GLib.Variant('s','')})))
candidates.append(("(a{sv}) with session_handle_token uuid", lambda: make_tuple_with_map({'session_handle_token': GLib.Variant('s', str(uuid.uuid4()))})))
candidates.append(("(a{sv}) with handle_token uuid", lambda: make_tuple_with_map({'handle_token': GLib.Variant('s', str(uuid.uuid4()))})))
candidates.append(("(a{sv}) with app_id", lambda: make_tuple_with_map({'app_id': GLib.Variant('s','nova-replay')})))
candidates.append(("(a{sv}) with parent_window", lambda: make_tuple_with_map({'parent_window': GLib.Variant('s','') })))
